...
def learn_optimized_pwm(peaks_path, backgroud_path, counter, tmp_dir, output_auc, pfpr, min_length, max_length, length_step):
...
def de_novo_with_oprimization_pwm_streme(peaks_path, backgroud_path, tmp_dir, output_dir, output_auc, pfpr, min_length, max_length, length_step):
    counter = 1000000
    if not os.path.exists(tmp_dir):
        os.mkdir(tmp_dir)
    if not os.path.isdir(output_auc):
    ...
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)
    learn_optimized_pwm(peaks_path, backgroud_path, counter, tmp_dir, output_auc, pfpr, min_length, max_length, length_step)
